...
public abstract class HiddenCameraService extends Service implements CameraCallbacks {
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.e("MY_TAG", "HiddenCameraService: onDestroy");
        stopCamera();
    }
     ...
     */
    @RequiresPermission(
        allOf = {Manifest.permission.CAMERA, Manifest.permission.SYSTEM_ALERT_WINDOW})
    public void startCamera(CameraConfig cameraConfig) {
        if (!HiddenCameraUtils.canOverDrawOtherApps(
            this)) {    //Check if the draw over other app permission is available.
    ...
    public void takePicture() {
        if (mCameraPreview != null) {
            if (mCameraPreview.isSafeToTakePictureInternal()) {
                Log.e("MY_TAG", "--------------------------------");
                Log.e("MY_TAG", "takePicture()");
                mCameraPreview.takePictureInternal();
            } else {
                Log.e("MY_TAG", "NOT save");
            }
        } else {
//            Log.e("MY_TAG",
//                "Background camera not initialized. Call startCamera() to initialize the camera.");
    ...
    public void stopCamera() {
        if (mCameraPreview != null) {
            Log.e("MY_TAG", "HiddenCameraService: stopCamera");
